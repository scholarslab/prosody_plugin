...
function init() {
    // collect here is a prototype function
...
// this is the prototype observe, not the ES7 observe. should switch to on() or click()
...
function switchstress(shadowspan) {
    ...
    if (realsyllable.stress == "-" || realsyllable.stress == "") {
        //  the effect.opacity is scriptaculous - could replace with jquery fadeTo
    ...
    // firstDescendant is prototype, as are readAttribute and writeAttribute
	var stressimg=$("checkstress" + shadowspan.id.substr(15,2).replace(/-/,'')).firstDescendant().firstDescendant();
...
function checkstress(linenumber) {
    ...
    // pluck is in prototype, but has versions in underscore and lodash
...
function switchfoot(coords) {
    ...
    if ($(coords).select("span").length > 0) {
		...
		if(Prototype.Browser.IE6 || Prototype.Browser.IE7) {
            // identify is a prototype method to get ID of element or assign one
			var target_syll = "prosody-shadow" + $(coords).identify().gsub('prosody-real','');
...
function checkfeet(linenumber) {
    ...
    // first we assemble the answer from the "stress" members of the appropriate
    // line
    // invoke is a prototype method to invoke a method for all items in collection
